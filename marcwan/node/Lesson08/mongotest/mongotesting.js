...
/* liste tous la collection d'albums */
    // var cursor = albums_coll.find();
/* liste avec restriction par id et classement croissant, passant les trois premier, et n'en afficher que trois */
    // var cursor = albums_coll.find({ albumid: "italy2012"}).sort({ filename: 1 }).skip(3).limit(3);
/* liste par date avec regex débute par 2010 */
    // var cursor = albums_coll.find({ date: {$regex: /^2010$/ }});

/* liste des photos par albumid et avec un nom de fichier précis */
    var cursor = photos_coll.find({ 
      albumid: "italy2012",
      filename: "picture_02.jpg"
    });
